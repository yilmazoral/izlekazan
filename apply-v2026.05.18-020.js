...
// Sürüm: v2026.05.18-020
// Kullanım: Bu dosyayı proje ana klasörüne koyup `node apply-v2026.05.18-020.js` çalıştırın.

const fs = require("fs");
const path = require("path");

const VERSION = "v2026.05.18-020";
const CACHE = "v20260518020";
...
function patchServer() {
  ...
  } else if (!s.includes("const APP_VERSION")) {
    s = `const APP_VERSION = "${VERSION}";\n` + s;
  }
  ...
  const fullLeaderboardBlockPattern = /function dateRange\(kind\) \{[\s\S]*?app\.get\("\/api\/public\/leaderboards"[\s\S]*?\n\}\);\s*/;
  const routeOnlyPattern = /app\.get\("\/api\/public\/leaderboards"[\s\S]*?\n\}\);\s*/;

  if (fullLeaderboardBlockPattern.test(s)) {
    s = s.replace(fullLeaderboardBlockPattern, leaderboardCode + "\n\n");
  } else if (routeOnlyPattern.test(s)) {
    s = s.replace(routeOnlyPattern, leaderboardCode + "\n\n");
  } else if (s.includes(marker)) {
    s = s.replace(marker, leaderboardCode + "\n\n" + marker);
  } else {
    throw new Error("/api/public/leaderboards endpointi eklenemedi; /api/admin/all işaretçisi bulunamadı.");
  }
...
function patchIndex() {
  ...
  if (h.includes("site-update.js") && !h.includes(`site-update.js?v=${CACHE}`)) {
    h = h.replace(/site-update\.js(\?v=[^"']+)?/g, `site-update.js?v=${CACHE}`);
  }
  if (!h.includes("site-update.js")) {
    const tag = `  <script src="site-update.js?v=${CACHE}"></script>\n`;
    if (/<\/body>/i.test(h)) h = h.replace(/<\/body>/i, tag + "</body>");
    else h += "\n" + tag;
  }
  h = h.replace(/<strong>v\d{4}\.\d{2}\.\d{2}-\d+<\/strong>/g, `<strong>${VERSION}</strong>`);
  h = h.replace(/İzleKazan\s+v\d{4}\.\d{2}\.\d{2}-\d+/g, `İzleKazan ${VERSION}`);
