...
// Sürüm: v2026.05.18-019
// Kullanım: Bu dosyayı proje ana klasörüne koyup `node apply-v2026.05.18-019.js` çalıştırın.

const fs = require("fs");
const path = require("path");

const VERSION = "v2026.05.18-019";
const CACHE = "v20260518019";
...
function patchServer() {
  ...
  const leaderboardBlockPattern = /function dateRange\(kind\) \{[\s\S]*?app\.get\("\/api\/public\/leaderboards", \(req, res\) => \{[\s\S]*?\n\}\);\s*/;
  if (leaderboardBlockPattern.test(s)) {
    s = s.replace(leaderboardBlockPattern, leaderboardCode + "\n\n");
  } else if (!s.includes('app.get("/api/public/leaderboards"')) {
    ...
    const marker = 'app.get("/api/admin/all", auth, adminOnly, (req, res) => {';
    const oldRoutePattern = /app\.get\("\/api\/public\/leaderboards",[\s\S]*?\n\}\);\s*/;
    if (oldRoutePattern.test(s)) s = s.replace(oldRoutePattern, leaderboardCode + "\n\n");
    else if (s.includes(marker)) s = s.replace(marker, leaderboardCode + "\n\n" + marker);
    else throw new Error("Mevcut liderlik endpointi bulundu ama güvenli şekilde değiştirilemedi.");
  }
