...
## v2026.05.18-021 — Temiz Tam Site Paketi

Tarih: 2026-05-18

### Yapılanlar
- v2026.05.18-020 düzeltmeleri ana proje dosyalarına doğrudan işlendi.
- `server.js`, `index.html`, `app.js`, `site-update.js`, `sw.js` ve `VERSION.json` sürüm/cache değerleri senkronlandı.
- Ana sayfada eski `leaderboardHome` alanı kaldırılıp tek `homeLeaderboards` alanı bırakıldı.
- Referans liderliği backend tarafında gerçek alt üye sayısına göre üretilecek şekilde güçlendirildi.
- 0 alt üyeli referans liderliği satırları backend ve frontend tarafında engellendi.
- Kullanılmayan update/apply dosyaları ve yardımcı parça dosyaları temizlendi.
- Kök dizindeki tekrar `film-afisleri-vitrin.png` kaldırıldı; `assets/film-afisleri-vitrin.png` korundu.
- `.gitignore`, `.env.example`, analiz raporu ve SHA256 kontrol dosyası eklendi.

### Korunanlar
- Supabase kalıcı veritabanı yapısı korunmuştur.
- Paketler Standart / VIP / CEO olarak korunmuştur.
- VIP Üye yalnızca 1. ve 2. seviyeden %20 referans kazancı sağlar; 3. seviye VIP içinde anılmaz.
- CEO Üye 1, 2 ve 3. seviyeden %30 kazanç bilgisiyle korunur.
- Paket akordeonları ve satın alma butonları korunmuştur.
- Mobil/PWA kullanım ve gizlilik maskeleme formatı korunmuştur.
- Cache sürümü `v20260518021` olarak güncellendi.
...
- Referans liderlik tablolarında `Tutar` yerine `Alt Üye Sayısı` gösterilecek şekilde frontend mantığı güncellendi.
- Paket kartları akordeon şeklinde açılır hale getirildi.
- Akordeon içeriğinde genel bilgi ve paket aksiyon butonu gösterildi.
- Paket fiyatının akordeon içinde tekrar edilmesi engellendi.
- VIP Üye içinde 3. seviye bilgisi ve olumsuz uyarı cümlesi kaldırıldı.

## v2026.05.18-018 — Mobil Liderlik Tablosu Düzeltmesi

- Mobilde liderlik tablolarında üçüncü sütunun görünmeme sorunu düzeltildi.
- Tutar / Alt Üye Sayısı sütunları sağa hizalı ve taşmadan görünecek şekilde ayarlandı.
- Liderlik kartlarının mobil padding, genişlik ve taşma davranışı düzenlendi.
- Alt mobil menünün içerikle çakışmasını azaltan güvenli alan düzeltmesi eklendi.

## v2026.05.18-017 — Liderlik Tabloları ve Paket Düzeni

- Ana sayfaya 8 liderlik tablosu yapısı eklendi.
- VIP Üye sadece 1. ve 2. seviyeden %20 referans kazancı alacak şekilde sınırlandı.
- 3. seviye yalnızca CEO Üye için bırakıldı.
- Paket akordeon detayında fiyat tekrarının kaldırılması planlandı.
