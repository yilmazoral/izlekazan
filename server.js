...
function recruiterRows(d, range, limit = 5) {
  ...
  for (const u of d.users) {
    ...
    const sponsorRaw = u.sponsorId || u.sponsorUserId || u.referrerId || u.parentId || u.invitedBy || u.referredBy;
    const sponsorId = resolveSponsorUserId(d, sponsorRaw);
    if (!sponsorId || !isInRange(u.createdAt || u.joinedAt || u.registeredAt, range)) continue;
    const row = map.get(sponsorId) || { userId: sponsorId, count: 0 };
    row.count += 1;
    map.set(sponsorId, row);
  }
  return [...map.values()]
    .filter(r => Number(r.count || 0) > 0)
    .sort((a, b) => b.count - a.count || String(a.userId).localeCompare(String(b.userId)))
    .slice(0, limit)
    .map((r, i) => ({ rank: i + 1, userId: r.userId, maskedName: leaderboardName(d, r.userId), count: r.count, altMemberCount: r.count, referralCount: r.count }));
}
...
app.get("/api/film-gateway", (req, res) => {
  ...
  try {
    ...
    if (!MOVIE_SITE_URL) return res.status(503).send(`<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;background:#080b14;color:#fff;font-family:Arial,sans-serif;display:grid;place-items:center;height:100vh;text-align:center;padding:24px}.box{max-width:560px;border:1px solid rgba(255,255,255,.14);border-radius:20px;padding:24px;background:rgba(255,255,255,.06)}h1{font-size:22px}p{color:#cbd5e1}</style></head><body><div class="box"><h1>Film adresi tanımlı değil</h1><p>Render Environment bölümünde MOVIE_SITE_URL değişkeni tanımlanmalıdır.</p></div></body></html>`);
